...
import { createTokenStore } from './thumbtack-token-store.mjs';
...
export function createThumbtackApiClient(options = {}) {
  const cfg = {
    apiBaseUrl: options.apiBaseUrl ?? thumbtackStagingApiBaseUrl,
  ...
  const tokenStore = options.tokenStore ?? (
    thumbtackTokenEncryptionKey && thumbtackTokenStorePath
      ? createTokenStore({ encryptionKey: thumbtackTokenEncryptionKey, storePath: thumbtackTokenStorePath })
      : null
  );
  ...
  function getNegotiations(businessId) {
    return request('GET', `/api/v4/businesses/${encodeURIComponent(businessId)}/negotiations`);
  }
