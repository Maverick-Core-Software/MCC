...
import {
  ...
  localModelUrl, localModel, llamaDirectUrl,
...
export async function localChat(messages, { system, model = localModel, signal, maxTokens = 2048, temperature = 0.2, retries = 1 } = {}) {
  ...
  for (let attempt = 0; attempt <= retries; attempt++) {
    try {
      ...
      // Guardian proxy unreachable — try direct llama-server.
      if (/ECONNREFUSED|fetch failed/i.test(msg)) {
        try {
          const direct = await fetch(`${llamaDirectUrl}/chat/completions`, {
            method: 'POST', signal,
            headers: { 'content-type': 'application/json' },
            body,
          });
        ...
        } catch { /* direct llama also unreachable */ }
      }
