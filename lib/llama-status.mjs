...
// Online/offline comes from guardian /__guardian/health llama_up (GLM on this PC),
// not from GET /v1/models — a fleet catalog 200 can list AIWA Nemotron while GLM sleeps.
...
export function pickLocalCoreModelEntry(models, configuredModel = localModel) {
  ...
  if (configuredModel) {
    ...
    if (hit && !isAiwaCatalogId(hit.id)) return hit;
  }
