...
import {
  ...
  anthropicApiKey, anthropicModel, openAiApiKey, geminiApiKey, geminiModel,
      ...
      try {
        ...
        if (result.success) {
          ...
          if (Array.isArray(result.unmatched) && result.unmatched.length) {
            sseWrite(res, `\n\n⚠️ ${result.unmatched.length} item(s) had no price book match and were added at $0 — set the price in HCP:\n${result.unmatched.map(u => `  • ${u}`).join('\n')}`);
          }
        } else {
