...
import {
  ...
  veniceQcModel, veniceModel, veniceBaseUrl, veniceApiKey,
} from './config.mjs';
...
import { callClaude, callPiRpc, openRouterChat, veniceChat } from './models.mjs';
...
async function handleBuildOrchestration(res, controller, prompt, histMsgs, ctxBlock, attachBlock = '', folderPaths = []) {
  ...
  try {
    ...
    const ragResp = await fetch(new URL('/estimate', ragUrl), {
      ...
      body: JSON.stringify({ message: prompt, history: [], top_k: 12 })
    });
...
PASS = correct, will run as-is. WARN = works but has notable issues. RETRY = will not run or is structurally wrong.`;
  ...
  // Venice QC primary → NIM fallback → OpenRouter last resort
  try {
    const r = await fetch(`${veniceBaseUrl}/chat/completions`, {
      ...
      body: JSON.stringify({ ...body, model: veniceQcModel }),
    });
    if (r.ok) {
...
async function delegateWriteToExecutor(task, staged, controller) {
  ...
  // Venice primary; OpenRouter GLM is the fallback when Venice is offline so OPS never goes dark.
  const editorOpts = { signal: controller.signal, temperature: 0.1, maxTokens: 4000 };
  let newContent;
  try {
    newContent = await veniceChat(editorMessages, { model: veniceModel, ...editorOpts });
  } catch (err) {
    if (err.name === 'AbortError') throw err;
    console.warn(`[executor] Venice failed — OpenRouter fallback: ${err.message}`);
    newContent = await openRouterChat(editorMessages, { model: openRouterExecutorModel, ...editorOpts });
  }
...
export async function handleBuildChat(req, res) {
  ...
  try {
    ...
    const ragResp = await fetch(new URL('/estimate', ragUrl), {
      ...
      headers: { 'content-type': 'application/json' },
      signal: ragCtrl.signal,
      body: JSON.stringify({ message: String(prompt).trim(), history: [], top_k: 12 })
    });
    clearTimeout(ragTimeout);
    if (ragResp.ok) {
      const ragData = await ragResp.json();
  ...
  for (let round = 0; round < 20 && !controller.signal.aborted; round++) {
    ...
    if (directive.done) {
      if (staged.files.length) {
        ...
        // Venice QC primary → NIM fallback
        let qcDone = false;
        if (!qcDone && veniceApiKey) {
          buildChatSseWrite(res, { type: 'status', text: 'Venice is verifying...' });
          try {
            let budget = 8000;
            ...
            const qcRes = await fetch(`${veniceBaseUrl}/chat/completions`, {
              method: 'POST',
            ...
            if (err.name === 'TimeoutError') buildChatSseWrite(res, { type: 'status', text: 'Venice QC timed out — checking NIM fallback.' });
          }
        }
        if (!qcDone && nimApiKey) {
...
async function spawnEstimatePipeline(payload, res, controller) {
  // Guard: estimate pipeline requires grizzly-hcp repo (stays on CartersPC).
  if (!hcpDir || !fs.existsSync(hcpDir)) {
    sseWrite(res, '\n⚠️ Estimate pipeline not available on this server. Please request estimates from the PC-hosted console.\n');
    res.end();
    return;
  }
  await new Promise(resolve => {
...
export async function handleChat(req, res) {
  try {
    ...
    // ASK → advisory Maverick brain (RAG tools, no live-HCP/write tools)
    if (mode === 'ask') {
    ...
    // REVIEW → Venice primary (if key set), Gemini first fallback (if key set), OpenRouter last.
    ...
    let upstream;
    // Try Venice first (OpenAI-compatible SSE shape)
    upstream = await fetch(`${veniceBaseUrl}/chat/completions`, {
      method: 'POST',
      signal: controller.signal,
      headers: { 'content-type': 'application/json', accept: 'text/event-stream', 'Authorization': `Key ${veniceApiKey}` },
      body: JSON.stringify({ model: veniceModel, messages, stream: true, temperature: 0.7, max_tokens: 1400 })
    });

    // Gemini first fallback when key is set
    if (!upstream.ok && useGemini) {
      try {
