...
import {
  ragUrl, llamaServerUrl, localModel, nimApiKey, nimModel, nimQcModel,
  anthropicApiKey, anthropicModel, openAiApiKey, openAiBaseUrl, geminiApiKey, geminiModel,
...
import { callClaude, callPiRpc } from './models.mjs';
  ...
  if (openAiApiKey) {
    try {
      const r = await fetch(`${openAiBaseUrl}/v1/chat/completions`, {
        ...
        body: JSON.stringify({ ...body, model: 'carter-gpt55' }),
      });
      if (r.ok) {
        const d = await r.json();
        const text = d.choices?.[0]?.message?.content?.trim();
  ...
  await new Promise(resolve => {
    ...
    proc.on('close', () => {
      ...
      try {
        const result = JSON.parse(stdout);
        if (result.success) {
          sseWrite(res, result.response);
        } else {
          sseWrite(res, `❌ Agent error: ${result.error}`);
        }
      } catch { sseWrite(res, '❌ Agent returned unexpected output.'); }
      resolve();
    });
...
export async function handleChat(req, res) {
  try {
    ...
    // ASK → intent detection first; then vision, RAG, Claude/Qwen fallback
    if (mode === 'ask') {
      // If there's a pending estimate in the frontend, any message is an edit request
      if (Array.isArray(pendingItems) && pendingItems.length) {
        await handleEstimateEdit(pendingItems, pendingCustomer || {}, prompt.trim(), res, controller);
        return;
      }
      // Detect "build it" intent → extract agreed items from conversation and show confirm card
      const lc = (prompt || '').toLowerCase();
      if (ESTIMATE_TRIGGERS.some(t => lc.includes(t))) {
        await handleEstimateFromAsk(histMsgs, prompt.trim(), res, controller);
        return;
      }
