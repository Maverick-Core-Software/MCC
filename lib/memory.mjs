...
// ponytail: assumes no symlink cycles inside the vault
...
export function loadBrainIndex(rootPath = brainPath) {
  ...
  let files;
  try {
    files = walkMarkdown(rootPath).sort();
  } catch (error) {
    // rootPath may exist but be a file, or a subdirectory may EPERM/EACCES (cloud placeholders).
    return { sourcePath: rootPath, state: 'error', memories: [], warnings: [`Brain vault unreadable: ${error.message}`] };
  }
  const warnings = [];
  const memories = files.flatMap((sourcePath) => {
    try {
      ...
      const memory = {
        ...
        description: redactMemoryBody(parsed.description || '') || firstBodyLine(body),
...
export function scoreMemories(memories, query) {
  ...
  // Spread intentionally drops the non-enumerable searchBody — results carry only the truncated body.
