// Agentic execution engine: path sandboxing, skill loading, workspace tree,
// the BUILD/OPS tool implementations (file IO, shell, web, docs, email, vision),
// and staged-run persistence. Security-sensitive — path + command allowlists live here.
import fs from 'node:fs';
import path from 'node:path';
import { spawn } from 'node:child_process';
import {
  workspacePath, skillsPath, stagingRoot, backupRoot, braveApiKey,
  anthropicApiKey, anthropicModel, BLOCKED_ABS_RE, BLOCKED_REL,
} from './config.mjs';
...
export const BLOCKED_COMMANDS = /(\brmdir\b|\bdel\b|\brd\b|\brm\b|\bformat\b|\bregdel\b|\bpowershell.*-enc\b|\bcurl.*\|\s*bash\b|npm\s+run\s+dev\b|npm\s+run\s+start\b|npm\s+start\b|next\s+dev\b|next\s+start\b|vite\b|nodemon\b)/i;
export const SAFE_COMMAND = /^(node|npm|npx|git|pm2|python|python3|dir|type|where|echo|ping|tracert|nslookup|ipconfig|mkdir|md|move|copy|ren|rename)\b/i;
...
export function persistStagedRun(staged) {
