...
export const llamaDirectUrl = process.env.LLAMA_DIRECT_URL || 'http://127.0.0.1:8081/v1';
...
// Customer-facing automation is deliberately a two-key enablement. The first
// key enables Maverick replies; the second confirms Thumbtack's native reply
// was disabled by an operator. HCP writes remain a separate future gate.
...
export const thumbtackAutomationFile = path.join(dataDir, 'thumbtack-automation.jsonl');
// Reply drafting runs a reasoning agent with pricebook tool calls; observed
// wall time is minutes, not seconds, so the ceiling defaults generously high.
const parsedAgentTimeout = Number.parseInt(process.env.THUMBTACK_AGENT_TIMEOUT_MS || '', 10);
export const thumbtackAgentTimeoutMs = Number.isFinite(parsedAgentTimeout) && parsedAgentTimeout > 0 ? parsedAgentTimeout : 600_000;
